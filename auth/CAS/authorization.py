...
class Authorization(object):
    ...
    def get_user_permissions(self, user):
        ...
        memberShipRecords = AuthMembership.objects(creator=self.client, user=user).only(
            "groups"
        )
        results = []
        for each in memberShipRecords:
            for group in each.groups:
                targetPermissionRecords = AuthPermission.objects(
                    creator=self.client, groups=group
                ).only("name")
    ...
    def del_permission(self, role, name):
        ...
        if not self.has_permission(role, name):
            return True
        targetGroup = AuthGroup.objects(role=role, creator=self.client).first()
        target = AuthPermission.objects(
            groups=targetGroup, name=name, creator=self.client
        ).first()
        if not target:
            return True
        target.delete()
