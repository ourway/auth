...
class BaseModel:
    ...
    def _update_modified(self, table: str, record_id: int):
        ...
        self._execute(f"UPDATE {table} SET modified = CURRENT_TIMESTAMP WHERE id = ?", (record_id,))
        self.conn.commit()
...
class AuthGroup(BaseModel):
    ...
    def create(self, creator: str, role: str, description: str = None) -> bool:
        """Create a new group"""
        try:
            self._execute(
    ...
    def get_all_by_creator(self, creator: str) -> List[Dict[str, Any]]:
        ...
        rows = self._fetch_all(
            "SELECT * FROM auth_group WHERE creator = ? AND is_active = 1",
            (creator,)
        )
...
class AuthMembership(BaseModel):
    ...
    def create_or_get(self, creator: str, user: str) -> int:
        ...
        cursor = self._execute(
            "INSERT INTO auth_membership (creator, user) VALUES (?, ?)",
            (creator, user)
        )
        self.conn.commit()
...
class AuthPermission(BaseModel):
    ...
    def create_or_get(self, creator: str, name: str) -> int:
        ...
        cursor = self._execute(
            "INSERT INTO auth_permission (creator, name) VALUES (?, ?)",
            (creator, name)
        )
        self.conn.commit()
        return cursor.lastrowid
