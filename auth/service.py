...
class AuthorizationService:
    ...
    def __init__(self, db: Session, client: str):
        self.db = db
        self.client = client
    
    def get_roles(self) -> List[Dict[str, Any]]:
        """Get all roles for the client"""
        groups = self.db.query(AuthGroup).filter(
            AuthGroup.creator == self.client,
            AuthGroup.is_active
        ).all()
        return [{"role": group.role, "description": group.description} for group in groups]
    
    def get_permissions(self, role: str) -> List[Dict[str, Any]]:
        """Get permissions for a role"""
        group = self.db.query(AuthGroup).filter(
            AuthGroup.creator == self.client,
            AuthGroup.role == role,
            AuthGroup.is_active
        ).first()
    ...
    def get_user_permissions(self, user: str) -> List[Dict[str, Any]]:
        ...
        membership = self.db.query(AuthMembership).filter(
    ...
    def get_user_roles(self, user: str) -> List[Dict[str, Any]]:
        ...
        return [{"user": membership.user, "role": group.role} for group in membership.groups if group.is_active]
    ...
    def which_roles_can(self, name: str) -> List[Dict[str, Any]]:
        ...
        permission = self.db.query(AuthPermission).filter(
    ...
    def add_role(self, role: str, description: str = None) -> bool:
        """Add a new role"""
        # Check if role already exists
        existing = self.db.query(AuthGroup).filter(
            AuthGroup.creator == self.client,
            AuthGroup.role == role
        ).first()
    ...
    def has_membership(self, user: str, role: str) -> bool:
        ...
        return any(group.role == role and group.is_active for group in membership.groups)
