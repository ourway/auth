...
def test_authorization_add_member(cas, member):
    ...
    if member == 'howard':
        ...
        howard_roles = [r for r in user_roles if r['user'] == 'howard' and r['role'] == 'other']
...
def test_authorization_nonexistent_role(cas):
    ...
    assert cas.del_permission('nonexistent', 'read') == True  # Should return True for non-existent
